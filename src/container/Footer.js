...
const Footer = () => {
    return ( <
        ...
        <
        a className = "transition-all duration-300 hover:text-indigo-300"
        href = "https://github.com/aayushx402/website"
        target = "_blank"
        rel = "noreferrer" >
        <
        p > Designed & amp; Created by Aa Yush < /p> <
        p > Built with React & amp; Tailwind CSS < /p> < /
        a > <
        /div> <
        ...
        div className = "flex space-x-6 items-center justify-center" > {
            social.map((item, index) => {
                ...
                return ( <
                    ...
                    href = { href }
                    key = { index }
                    className = { `text-base ${color} hover:text-white border-2 p-2 border-indigo-600 hover:bg-indigo-600 rounded-full transition-all duration-300` } > { icon } <
                    /a>
                );
            })
        } <
        ...
        <
        p > Copyright & copy; 2024 Aayush All Rights Reserved. < /p> < /
        div > <
        /div> < /
        footer >
    );
